...
def histogram_all(data, feature_names, label_name):
    ...
    for column_name in feature_names + [label_name]:
        # ax is the plot itself n is the highest y point so that all the y axis are the same
    ...
    fig.tight_layout()
    figure_name = "plots/showcase_histograms.png"
...
def scatter_all(data, feature_names, label_name):
    ...
    fig.tight_layout()
    figure_name = "plots/showcase_scatters.png"
    fig.savefig(figure_name)
    plt.close(fig)
    return
