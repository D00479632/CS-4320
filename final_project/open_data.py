...
def load_diabetes_data(number):
    """
    number == 1 -> load training data (dropped_train.csv)
    number == 2 -> load validation data (dropped_validate.csv)
    number == -1 -> load test data (dropped_test.csv)
    """
    import pandas as pd
    
    if number == 1:
        # Training data
        data = pd.read_csv('data/preprocessed-dropped-train.csv')
    elif number == 2:
        # Validation data
        data = pd.read_csv('data/preprocessed-dropped-validate.csv')
    elif number == -1:
        # Test data
        data = pd.read_csv('data/preprocessed-dropped-test.csv')
    else:
        raise ValueError("Invalid number parameter. Use 1 for training, 2 for validation, or -1 for test data.")
