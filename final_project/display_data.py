...
def display_feature_histograms(my_args, data):
    ...
    for feature_column in feature_columns:
        if feature_column in data.columns:
            ...
            figure_name = f"plots/{basename}-histogram-{feature_column}.png"
    ...
    if label_column:
        ...
        basename = get_basename(my_args.data_file)
        figure_name = f"plots/{basename}-histogram-{label_column}.png"
        fig.savefig(figure_name)
        plt.close(fig)
...
def display_label_vs_features(my_args, data):
    ...
    for feature_column in feature_columns:
        if feature_column in data.columns:
            ...
            ax = fig.add_subplot(1, 1, 1)  
            ax.scatter(col_df[feature_column], col_df[label_column], s=10, alpha=0.5)  # Increased size and added transparency
            ax.set_xlabel(feature_column)  
            ax.set_ylabel(label_column)  
            ax.locator_params(axis='both', tight=True, nbins=5)  
            ax.set_ylim(bottom=-0.1, top=y_max + 0.1)  # Adjust y-axis limits slightly for better visibility

            basename = get_basename(my_args.data_file)  
            figure_name = f"plots/{basename}-scatter-{feature_column}.png"  
            fig.savefig(figure_name)  
            plt.close(fig)  
...
def parse_args(argv):
    ...
    parser.add_argument('--label', '-l', default="diabetes", type=str, help="column name for label")  # Define label argument
